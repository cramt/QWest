...
namespace QWest.DataAccess.Tests {
    ...
    public class DaoPostSpecSetupAndTearDown {
        ...
        [OneTimeTearDown]
        public void Setup() {
            ConnectionWrapper.Instance.Use("DELETE FROM images; DELETE FROM users_friendships; DELETE FROM users_friendship_requests; DELETE FROM users_groups; DELETE FROM users; DELETE FROM groups; DELETE FROM posts", stmt => stmt.ExecuteNonQueryAsync()).Wait();
        }
    }
}
