...
BEGIN
	...
	BEGIN
		DELETE FROM @temp;
		INSERT INTO @temp SELECT sub.id, sub.alpha_2, sub.alpha_3, sub.name, sub.official_name, sub.common_name, sub.type, sub.numeric, sub.super_id FROM geopolitical_location sub INNER JOIN geopolitical_location super ON sub.super_id = super.id INNER JOIN @curr c ON super.id = c.id
